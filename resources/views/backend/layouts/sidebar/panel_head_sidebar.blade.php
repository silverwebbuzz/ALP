...
@if(Auth::user()->role_id == 8)
    <nav id="sidebar" class="@if(!empty(Session::get('sidebar'))){{Session::get('sidebar')}}@endif" style="background-color:{{$RoleBasedColor['background_color']}};">
        ...
        <ul class="list-unstyled components mb-5">
            <li class="{{ (request()->is('panel-head/dashboard') || request()->is('dashboard')) ? 'active' : ''}}">
                {{-- <a href="{{ route('panel-head.dashboard') }}"> --}}
                    ...
                    <div class="collapse {{(
                        ...
                        <ul class="flex-column pl-2 nav">
                            ...
                            @if(in_array('assignment_or_test_read', $permissions))
                                <li class="nav-item {{ (request()->is('myteaching/assignment-exercise') || (isset($menuItem) && ($menuItem == 'exercise'))) ? 'active': '' }}">
