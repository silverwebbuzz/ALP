                                    ...
                                    <div class="d-flex review-question-main tab-content-wrap">
                                        <div class="review-question-left-section">
                                            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist" aria-orientation="vertical">
                                                @foreach($learningObjectiveData as $key => $learningObjectives)
                                                    <span class="w-10 nav-link ordering-unit-objective Indexing d-inline-block @if($key==0) active @endif" >{{substr($LearningUnits[0]['index'],0,1).'.'.($key + 1)}}</span>
                                                    <li class="w-90 ordering-unit-objective nav-item">
                                                        <p class="nav-link learning_objective_ordering @if($key==0) active @endif" data-id={{$learningObjectives['id']}} > {{ $learningObjectives['title_'.app()->getLocale()] }} ({{$learningObjectives['foci_number']}})</p>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        </div>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on("change", "#ordering_leraningUnit_id", function () {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    strand_id: [{{$strandsIds}}],
                    learning_unit_id : $("#ordering_leraningUnit_id").val()
                },
                success: function (response) {
                    ...
                    if (data.data) {
                        ...
                        $.each(data.data.LearningObjectives, function(K,Q) {
                            var tab_active = '';
        ...
        $(document).on('click','.learning_objective_ordering',function(){
            position = ((position1_array.indexOf($(this).data('id'))) ); 
            $(".learning_objective_ordering").removeClass("active");
