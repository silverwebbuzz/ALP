                                             ...
                                             @foreach($ExamsData['test_list'] as $selfLearningTest)
                                                 ...
                                                 <tr>
                                                     ...
                                                     @if(isset($examArray['attempt_exams']) && in_array($studentId,array_column($examArray['attempt_exams'],'student_id')))
                                                         @php
                                                         $accuracy = App\Helpers\Helper::getAccuracy($selfLearningTest->id, $studentId);
                                                     ...
                                                     <td>
                                                         @php
                                                         $progressQuestions = App\Helpers\Helper::getQuestionDifficultiesLevelPercent($selfLearningTest->id,$studentId);
                                                         @endphp
                                                     ...
                                                     <td class="btn-edit">
                                                         @if(in_array('attempt_exam_update', $permissions))
                                                             @if(!isset($examArray['attempt_exams']) || (isset($examArray['attempt_exams']) && !in_array($studentId,array_column($examArray['attempt_exams'],'student_id'))) && $selfLearningTest->status == 'publish' && (App\Helpers\Helper::CheckExamStudentMapping($selfLearningTest->id) == false))
                                                         ...
                                                         {{-- @if(in_array('result_management_read', $permissions)) --}}
                                                             ...
                                                             <a href="{{route('adminexams.result',['examid' => $selfLearningTest->id, 'studentid' => $studentId])}}" class="view-result-btn" title="{{__('languages.result_text')}}">
