		...
		<div class="sm-right-detail-sec pl-5 pr-5">
			<div class="container-fluid">
				...
				<form class="displayStudentStudyForm" id="displayStudentStudyForm" method="GET">
					...
					<div class="row">
						<div class="col-lg-2 col-md-2 grade-class-section-filtration">
							<div class="select-lng pb-2">
								...
								<select class="form-control" data-show-subtext="true" data-live-search="true" name="grade_id[]" id="student_multiple_grade_id" multiple required >
									...
									<option value="{{$gradeData->id}}" @if($grade_id){{ in_array($gradeData->id,$grade_id) ? 'selected' : '' }} @endif>{{ $gradeData->name}}</option>
									@endforeach
									@endif
								</select>
							</div>
						</div>
						<div class="col-lg-2 col-md-3 grade-class-section-filtration">
                            <div class="select-lng pb-2">
						...
						{{-- For Group Filter --}}
						<div class="col-lg-2 col-md-3 group_filter_section">
                            <div class="select-lng pb-2">
                            	<label for="users-list-role">{{ __('languages.group') }}</label>
                                <select name="group_filter[]" class="form-control" id="group_filter" multiple>
									@if(!empty($peerGroupData))
										@foreach($peerGroupData as $peergroup)
											<option value="{{$peergroup->id}}" @if(!empty($filterPeerGroupIds)) {{ in_array($peergroup->id,$filterPeerGroupIds) ? 'selected' : '' }} @endif>{{$peergroup->group_name}}</option>
										@endforeach
									@endif
                                </select>
                            </div>
						...
						<div class="col-lg-2 col-md-3">
                            <div class="select-lng pb-2">
                            	<label for="users-list-role">{{ __('languages.date') }}</label>
                                <select name="date_filter" class="form-control" id="date_filter" >
                                	<option>{{__('languages.select')}}</option>
                                    <option value="this_week" @if(request('date_filter')=="this_week") selected @endif>{{__('languages.this_week')}}</option>
                                    <option value="this_month" @if(request('date_filter')=="this_month" || request('date_filter')=="") selected @endif>{{__('languages.this_month')}}</option>
                                    <option value="last_two_weeks"  @if(request('date_filter')=="last_two_weeks") selected @endif>{{__('languages.last_two_weeks')}}</option>
                                    <option value="last_three_months"  @if(request('date_filter')=="last_three_months") selected @endif>{{__('languages.last_three_months')}}</option>
                                    <option value="last_six_months"  @if(request('date_filter')=="last_six_months") selected @endif>{{__('languages.last_six_months')}}</option>
                                    <option value="last_year"  @if(request('date_filter')=="last_year") selected @endif>{{__('languages.last_year')}}</option>
                                </select>
                            </div>
                        </div>
						<div class="col-lg-3 col-md-3">
                            <div class="select-lng pb-2">
                                ...
                                <input name="title" class="form-control" id="title" placeholder="{{__('languages.title')}}" value="{{request()->title}}"/>
							</div>
                        </div>
						
						{{-- <div class="col-lg-2 col-md-4">
							<div class="select-lng pt-1 pb-1">
				...
				<div class="row question_difficulty_level_colors">
					...
					@if(!empty($difficultyLevels))
						...
						@php
                        if($difficultLevel->difficulty_level == 4 || $difficultLevel->difficulty_level == 5){
                            continue;
                        }
                        @endphp
						<div class="question_difficulty_level_colors_sec">
				...
				<div class="modal-body">
					<div class="row pb-3">
						<div class="col-md-4 text-center">
							<button type="button" class="btn btn-primary class-ability-graph-btn" data-graphtype="my-class" data-classAbilityIsGroup="false"><span class="my_class_group_button">{{__('languages.my_class.my_classes')}}</span></button>
						</div>
						<div class="col-md-4 text-center">
