            ...
            <div class="sm-right-detail-sec pl-5 pr-5">
                ...
                <div class="wrapper-1">
                      <div class="wrapper-2">
                        ...
                        @endif
                        @if(($exam['exam_type'] == 2))
                          <a href="{{route('getStudentExerciseExamList',Auth::user()->id)}}"><button class="go-home">Go Back to {{($exam['exam_type'] == 2) ? __('languages.exercise') : __('languages.test_text')}} List</button>
                        @endif
                        @if(($exam['exam_type'] == 3))
                          <a href="{{route('getStudentTestExamList',Auth::user()->id)}}"><button class="go-home">Go Back to {{($exam['exam_type'] == 2) ? __('languages.exercise') : __('languages.test_text')}} List</button>
                        @endif
                      </div>
                    </div>
                </div>
