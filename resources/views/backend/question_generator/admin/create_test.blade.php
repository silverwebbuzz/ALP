    ...
    <div id="content" class="pl-2 pb-5">
        ...
        <div class="sm-right-detail-sec pl-5 pr-5">
            <div class="container-fluid">
                ...
                <form name="question-generator" id="question-generator" action="{{ route('super-admin.generate-questions') }}" method="POST" onsubmit="ShowLoader();">
                    @csrf
                                ...
                                <section class="form-steps step1">
                                    <div class="form-row">
                                        <div class="form-group col-md-6 mb-50">
                                            {{-- <label class="text-bold-600">{{__('languages.question_generators_menu.use_of_mode')}}</label> --}}
                                            <label class="text-bold-600">{{__('languages.mode')}}</label>
                                            <select name="use_of_modes" class="form-control select-option" id="use_of_modes">
                                                <option value="">{{__('languages.select_mode')}}</option>
                                                <option value="1" @if(request()->get('type') == 1) selected @endif>{{__('languages.question_generators_menu.as_a_test_exercise')}}</option> 
                                                <option value="2" @if(request()->get('type') == 2) selected @endif>{{__('languages.challenge_exercise_template')}}</option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group col-md-6 mb-50">
                                            <label id="test_start_date">{{ __('languages.question_generators_menu.start_date') }}</label>
                                            <div class="input-group date">
                                                <input type="text" class="form-control date-picker" id="start_date" name="start_date" value="{{date('d/m/Y')}}" placeholder="{{__('languages.question_generators_menu.select_start_date')}}" autocomplete="off">
                                        ...
                                        <div class="form-group col-md-6 mb-50">
                                            <label id="test_end_date">{{ __('languages.question_generators_menu.end_date') }}</label>
                                            <div class="input-group date">
                                                <input type="text" class="form-control end-date-picker" id="end_date" name="end_date" value="{{date('d/m/Y',strtotime("+1 days"))}}" placeholder="{{ __('languages.question_generators_menu.select_end_date') }}" autocomplete="off">
                                                <div class="input-group-addon input-group-append">
                                                    <div class="input-group-text">
                                                        <i class="glyphicon glyphicon-calendar fa fa-calendar"></i>
                                                    </div>
...
$(function (){
    ...
    */
    ...
    //         if(endOptionSelectedStartTimeIndex <= selectedStartTimeIndex && $(".date-picker").val() == $(".end-date-picker").val() ){
...
<script type="text/javascript">
    $(document).ready(function () {
        $(document).on("change", "#use_of_modes", function () {
            if($("#use_of_modes").val() == 2){
                $("#test_start_date").text(NOW);
                $("#test_end_date").text(DEADLINE);
                $("#start_date").attr("placeholder",NOW);
                $("#end_date").attr("placeholder",DEADLINE);
                $('#select-report-date option[value=after_submit]').prop('selected', 'selected').change();
            }else{
                $("#test_start_date").text(START_DATE);
                $("#test_end_date").text(END_DATE);
                $("#start_date").attr("placeholder",START_DATE);
                $("#end_date").attr("placeholder",END_DATE);
                $('#select-report-date option[value=end_date]').prop('selected', 'selected').change();
            }
        });
    });
</script>
