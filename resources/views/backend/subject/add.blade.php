            ...
            <div class="sm-right-detail-sec pl-5 pr-5">
				<div class="container-fluid">
					<div class="row">
						<div class="col-md-12">
							<div class="sec-title">
								<h2 class="mb-4 main-title">{{__('languages.subjects.add_new_subject')}}</h2>
							</div>
							<hr class="blue-line">
						</div>
					</div>
					<div class="sm-add-user-sec card">
                        ...
                        @endif
							...
							<form class="subject-form" method="post" id="addSubjectsForm"  action="{{ route('subject.store') }}">
							@csrf()

                                <div class="form-row select-data">
                                    
                                    <div class="form-group col-md-6 mb-50">
                                        <label class="text-bold-600" for="name">{{ __('languages.name') }}</label>
                                        <input type="text" class="form-control" name="name" id="name" placeholder="{{ __('languages.name') }}" value="{{old('name')}}">
                                        @if($errors->has('name'))<span class="validation_error">{{ $errors->first('name') }}</span>@endif
                                    </div>
                                    <div class="form-group col-md-6 mb-50">
                                        <label class="text-bold-600" for="code">{{ __('languages.code') }}</label>
