...
class com_wiris_util_json_parser_JsonParse {
	...
	static function buildErrorMessage($stateStack, $message) {
		...
		$jsonTrace = com_wiris_util_json_parser_JsonParse_1($jsonTrace, $message, $stateStack);
...
function com_wiris_util_json_parser_JsonParse_1(&$jsonTrace, &$message, &$stateStack) {
	if($jsonTrace === "") {
		return "<root>";
	} else {
