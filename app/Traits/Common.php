...
trait Common {
    ...
    public function UpdateUsedQuestionAnswerCounts($QuestionId, $UpdateType='question_count', $AnswerId=0){        
        if(UsedQuestionAnswerCount::where(cn::USED_QUESTION_ANSWER_QUESTION_ID_COL,$QuestionId)->doesntExist()){
            UsedQuestionAnswerCount::Create([cn::USED_QUESTION_ANSWER_QUESTION_ID_COL => $QuestionId]);
        }
        
        $UsedQuestionAnswerCount = UsedQuestionAnswerCount::where(cn::USED_QUESTION_ANSWER_QUESTION_ID_COL,$QuestionId)->first();
        if($UpdateType == 'question_count'){
            $currentQuestionCount = ($UsedQuestionAnswerCount->{cn::USED_QUESTION_ANSWER_QUESTION_COUNT_COL} + 1);
            UsedQuestionAnswerCount::where(cn::USED_QUESTION_ANSWER_QUESTION_ID_COL,$QuestionId)
            ->Update([
                cn::USED_QUESTION_ANSWER_QUESTION_COUNT_COL => ($UsedQuestionAnswerCount->{cn::USED_QUESTION_ANSWER_QUESTION_COUNT_COL} + 1) ?? 0
            ]);
        }

        // Update all answers counts
        if(isset($AnswerId) && !empty($AnswerId)){
            if(isset($UsedQuestionAnswerCount) && !empty($UsedQuestionAnswerCount)){
                if($UpdateType == 'answer_count'){
                    switch($AnswerId){
                        case 1:
                            UsedQuestionAnswerCount::where(cn::USED_QUESTION_ANSWER_QUESTION_ID_COL,$QuestionId)
                            ->Update([
                                cn::USED_QUESTION_ANSWER_ANSWER1_COUNT_COL => ($UsedQuestionAnswerCount->{cn::USED_QUESTION_ANSWER_ANSWER1_COUNT_COL} + 1) ?? 0
                            ]);
                            break;
                        case 2:
                            UsedQuestionAnswerCount::where(cn::USED_QUESTION_ANSWER_QUESTION_ID_COL,$QuestionId)
                            ->Update([
                                cn::USED_QUESTION_ANSWER_ANSWER2_COUNT_COL => ($UsedQuestionAnswerCount->{cn::USED_QUESTION_ANSWER_ANSWER2_COUNT_COL} + 1) ?? 0
                            ]);
                            break;
                        case 3:
                            UsedQuestionAnswerCount::where(cn::USED_QUESTION_ANSWER_QUESTION_ID_COL,$QuestionId)
                            ->Update([
                                cn::USED_QUESTION_ANSWER_ANSWER3_COUNT_COL => ($UsedQuestionAnswerCount->{cn::USED_QUESTION_ANSWER_ANSWER3_COUNT_COL} + 1) ?? 0
                            ]);
                            break;
                        case 4:
                            UsedQuestionAnswerCount::where(cn::USED_QUESTION_ANSWER_QUESTION_ID_COL,$QuestionId)
                            ->Update([
                                cn::USED_QUESTION_ANSWER_ANSWER4_COUNT_COL => ($UsedQuestionAnswerCount->{cn::USED_QUESTION_ANSWER_ANSWER4_COUNT_COL} + 1) ?? 0
                            ]);
                            break;
                        case 5:
                            UsedQuestionAnswerCount::where(cn::USED_QUESTION_ANSWER_QUESTION_ID_COL,$QuestionId)
                            ->Update([
                                cn::USED_QUESTION_ANSWER_ANSWER5_COUNT_COL => ($UsedQuestionAnswerCount->{cn::USED_QUESTION_ANSWER_ANSWER5_COUNT_COL} + 1) ?? 0
                            ]);
                            break;
                    }
                }
        ...
        if(Auth::user()){
            ...
            switch (Auth::user()->roles['role_slug']) {
                case "superadmin":
                    ...
                    break;
                case "principal":
                    $redirectUrl = config()->get('app.url').'dashboard';
  ...
  public static function  TotalDurationCalculation($examTimes){
    ...
    */
    ...
    public static function getStudentResultSummary($request){
        ...
        $header = [
            __('languages.class'),
            __('languages.student_no_within_class')
        ];
        ...
        $correctAnswerArray = [
            '',
            ($ExamData->exam_type == 1) ? __('languages.correct_answer') : __('languages.correct_answers')
        ];

        //Set Sub Main Header
        $QuestionHeaders = [
            '',
            __('languages.questions')
        ];
        if(!empty($ExamData)){
    ...
    public static function GetStudentDataByCurriculumYear($YearId,$UserId){
     ...
     /**
     * USE : Get Student Attempted Test Count by Curriculum Year
     */    
    public static function GetTestCountByCurriculumYear($YearId,$UserId){
        $TestCount = AttemptExams::with('exam')->whereHas('exam',function ($q) use($YearId){
                                $q->where('exam_type',3)
                                ->where('curriculum_year_id',$YearId);
                            })
                            ->where('student_id',$UserId)
                            ->count();
        return strlen($TestCount > 1) ? $TestCount : '0'.$TestCount;
    }

    /**
     * USE : Get Student Attempted Excercise Count by Curriculum Year
     */    
    public static function GetExerciseCountByCurriculumYear($YearId,$UserId){
        $ExerciseCount = AttemptExams::with('exam')->whereHas('exam',function ($q) use($YearId){
                                $q->where('exam_type',2)
                                ->where('curriculum_year_id',$YearId);
                            })
                            ->where('student_id',$UserId)
                            ->count();        
        return strlen($ExerciseCount > 1) ? $ExerciseCount : '0'.$ExerciseCount;
    }

    /**
     * USE : Get Student Attempted Self-Learning Count by Curriculum Year
     */    
    public static function GetSelfLearningCountByCurriculumYear($YearId,$UserId){
        $SelfLearningCount = AttemptExams::with('exam')->whereHas('exam',function ($q) use($YearId){
                                $q->where('exam_type',1)
                            ...
                            ->where('student_id',$UserId)
                            ->count();
        return strlen($SelfLearningCount > 1) ? $SelfLearningCount : '0'.$SelfLearningCount;
    }

    /**
     * USE : Get Student Attempted Test Count by Curriculum Year
     */    
    public static function GetAiBasedAssessmentCountByCurriculumYear($YearId,$UserId){
        $AiBasedAssessmentCount = AttemptExams::with('exam')->whereHas('exam',function ($q) use($YearId){
                                $q->where('exam_type',1)
                                ->where('reference_no','Like','Z%')
                                ->where('curriculum_year_id',$YearId);
                            })
                            ->where('student_id',$UserId)
                            ->count();
        return strlen($AiBasedAssessmentCount > 1) ? $AiBasedAssessmentCount : '0'.$AiBasedAssessmentCount;
    }
    ...
    public function getPercentageOfBasedOnAllUsedQuestionAnswerAllTest($ExamId){
        if(isset($ExamId) && !empty($ExamId)){
            $percentage = array();
            $ExamData = Exam::find($ExamId);
            if(isset($ExamData) && !empty($ExamData)){
                // Get the question ids
                if(isset($ExamData->{cn::EXAM_TABLE_QUESTION_IDS_COL}) && !empty($ExamData->{cn::EXAM_TABLE_QUESTION_IDS_COL})){
                    $QuestionIds = explode(',',$ExamData->{cn::EXAM_TABLE_QUESTION_IDS_COL});
    ...
    /**
     * USE : Filter on Date on Reports
     */
    public function filterOnDateInReport($Query,$filterOnDate,$model=""){

        if($model=="Exam"){
            switch($filterOnDate){
                case  'this_week':
                        $Query->whereBetween(cn::EXAM_TABLE_FROM_DATE_COLS, [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()]);
                        break;
                case  'last_two_weeks':
                        $Query->where(cn::EXAM_TABLE_FROM_DATE_COLS,'>=',Carbon::now()->subDays(14));
                        break;
                case  'last_three_months':
                        $Query->whereBetween(cn::EXAM_TABLE_FROM_DATE_COLS, [Carbon::now()->subMonth(3), Carbon::now()]);
                        break;
                case  'last_six_months':
                    $Query->whereBetween(cn::EXAM_TABLE_FROM_DATE_COLS, [Carbon::now()->subMonth(6), Carbon::now()]);
                    break;
                case  'last_year':
                    $Query->where(cn::EXAM_TABLE_FROM_DATE_COLS,'>=',Carbon::now()->subDays(365));
                    break;
                case 'this_month':
                    $Query->where(cn::EXAM_TABLE_FROM_DATE_COLS, '>=', Carbon::now()->subdays(30));
                    break;
            }
        }
        if($model==""){
            switch($filterOnDate){
                case  'this_week':
                        $Query->whereHas('exams',function($q){
                            $q->whereBetween(cn::EXAM_TABLE_FROM_DATE_COLS, [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()]);
                        });
                        break;
                case  'last_two_weeks':
                        $Query->whereHas('exams',function($q){
                            $q->where(cn::EXAM_TABLE_FROM_DATE_COLS,'>=',Carbon::now()->subDays(14));
                        });
                        break;
                case  'last_three_months':
                        $Query->whereHas('exams',function($q){
                            $q->whereBetween(cn::EXAM_TABLE_FROM_DATE_COLS, [Carbon::now()->subMonth(3), Carbon::now()]);
                        });
                        break;
                case  'last_six_months':
                    $Query->whereHas('exams',function($q){
                        $q->whereBetween(cn::EXAM_TABLE_FROM_DATE_COLS, [Carbon::now()->subMonth(6), Carbon::now()]);
                    });
                    break;
                case  'last_year':
                    $Query->whereHas('exams',function($q){
                        // $q->whereBetween(cn::EXAM_TABLE_FROM_DATE_COLS, [Carbon::now()->subYear(), Carbon::now()]);
                        // $q->whereYear(cn::EXAM_TABLE_FROM_DATE_COLS, date('Y', strtotime('-1 year')));
                        $q->where(cn::EXAM_TABLE_FROM_DATE_COLS,'>=',Carbon::now()->subDays(365));
                    });
                    break;
                case 'this_month':
                    $Query->whereHas('exams',function($q){
                        // $q->whereMonth(cn::EXAM_TABLE_FROM_DATE_COLS, Carbon::now()->month);
                        $q->where(cn::EXAM_TABLE_FROM_DATE_COLS, '>=', Carbon::now()->subdays(30));
                    });
                    break;
            }
        }
    ...
    // Get all Group IDs detail.
    public function getGroupData($groupIds){
        $GroupDetail = PeerGroup::whereIn('id',$groupIds)->get();
        if(!empty($GroupDetail)){
            return $GroupDetail->toArray();
        }
        return [];
    }

    /***
     * USE : Get All Role Based Users for particular School
     */
    public function getAllUsersCount($yearId){
        $userCount = [];
        $Roles = Role::All();
        foreach ($Roles as $key => $roles) {
            if(Auth::user()->role_id == 2 && $roles->role_slug == 'student'){
                $students = self::getAssignClassStudent();
                $userCount[$roles->role_slug] = $students->count();
            }
            $userCount[$roles->role_slug] = User::whereHas('roles', function($query) use($roles,$yearId) {               
                $query->where([
    ...
    /***
     * USE : Get All Role Based Exams for particular School
     */
    public function getAllExamsCount($yearId){
        $examCount = [];
        $examType = [
            'self_learning' => [
                'id' => 1,
                'reference_no_first_character' =>   'S' 
            ],
            'ai_based_assessment' => [
                'id' => 1,
                'reference_no_first_character' =>   'Z' 
            ],
            'exercise' => [
                'id' => 2,
                'reference_no_first_character' =>   'E' 
            ],
            'test' => [
                'id' => 3,
                'reference_no_first_character' =>   'T' 
            ],
        ...
        foreach ($examType as $key => $type) {
            if(Auth::user()->role_id == 7 || Auth::user()->role_id == 8 || Auth::user()->role_id == 9){
                ...
                $ExamCount[$key] = MyTeachingReport::where(function($query) use($gradesListIdArr, $TeacherAssignedClass,$PeerGroupIds){
                                    ...
                                    ->with('exams')
                                    ->whereHas('exams',function($q) use($type){
                                        $q->where('exam_type', $type['id'])
                                        ->where('reference_no','Like',$type['reference_no_first_character'].'%');
                                    })
                                    ->count();
    ...
    /***
     * USE : Get All Role Based Form/Grade for particular School
     */
    public function getAllGradesCount($yearId){
        $gradesCount = [];
        if(Auth::user()->role_id == 7 || Auth::user()->role_id == 8 || Auth::user()->role_id == 9){
            $gradesCount = GradeSchoolMappings::where(['curriculum_year_id' => $yearId,
                                                        'school_id'         => Auth::user()->school_id,
                                                        'status'            => 'active'
                                                    ])->count();
        }else if(Auth::user()->role_id == 2){
            $gradesCount =   TeachersClassSubjectAssign::where([
                                cn::TEACHER_CLASS_SUBJECT_TEACHER_ID_COL => Auth::user()->{cn::USERS_ID_COL},
                                cn::TEACHER_CLASS_SUBJECT_CURRICULUM_YEAR_ID_COL => $this->GetCurriculumYear()
                            ])
                            ->with('getClass')
    ...
    /***
     * USE : Get All Role Based Classes for particular School
     */
    public function getAllClassesCount($yearId){
        $classesCount = [];
        if(Auth::user()->role_id == 7 || Auth::user()->role_id == 8 || Auth::user()->role_id == 9){
            $classesCount = GradeClassMapping::where(['curriculum_year_id' => $yearId,
                                                        'school_id'         => Auth::user()->school_id,
                                                        'status' => 'active'
                                                    ])->count();
        }elseif(Auth::user()->role_id == 2){
            $classesCount =TeachersClassSubjectAssign::where([
                            cn::TEACHER_CLASS_SUBJECT_TEACHER_ID_COL => Auth::user()->{cn::USERS_ID_COL},
                            cn::TEACHER_CLASS_SUBJECT_CURRICULUM_YEAR_ID_COL => $this->GetCurriculumYear()
                        ])
                        ->pluck(cn::TEACHER_CLASS_SUBJECT_CLASS_NAME_ID_COL)
     ...
     /***
     * USE : Get All Role Based Group for particular School
     */
    public function getAllGroupCount($yearId){
        $groupCount = [];
        if(Auth::user()->role_id == 7 || Auth::user()->role_id == 8 || Auth::user()->role_id == 9){
            $groupCount = PeerGroup::where(['curriculum_year_id' => $yearId,
                                                'school_id'         => Auth::user()->school_id,
                                                'status'            => 1,
                                                'group_type'         => 'group'
                                            ])->count();
    ...
    /***
     * USE : Get All Role Based Peer Group for particular School
     */
    public function getAllPeerGroupCount($yearId){
        $peerGroupCount = [];
        if(Auth::user()->role_id == 7 || Auth::user()->role_id == 8 || Auth::user()->role_id == 9){
            $peerGroupCount = PeerGroup::where(['curriculum_year_id' => $yearId,
                                                'school_id'         => Auth::user()->school_id,
                                                'status'            => 1,
                                                'group_type'         => 'peer_group'
                                            ])->count();
        }elseif(Auth::user()->role_id == 2){
            $peerGroupCount = PeerGroup::where(['curriculum_year_id' => $yearId,
                                                'school_id'         => Auth::user()->school_id,
    ...
    public function getAssignClassStudent(){
        $gradeClassId = array();
