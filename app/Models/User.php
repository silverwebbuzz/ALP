...
class User extends Authenticatable 
{
    ...
    /**
     * USE : Get All Attempted Test Exam Count
     */
    public function getTestCountAttribute(){
        $TestCount = null;
        $UserData = Self::find($this->id);
        if(!empty($this->id) && isset($UserData->role_id) && $UserData->role_id == cn::STUDENT_ROLE_ID){
            $TestCount = $this->GetTestCountByCurriculumYear($this->GetCurriculumYear(),$this->id);
            return $TestCount;
        }
    }

     /**
     * USE : Get All Attempted Exercise Exam Count
     */
    public function getExerciseCountAttribute(){
        $ExerciseCount = null;
        $UserData = Self::find($this->id);
        if(!empty($this->id) && isset($UserData->role_id) && $UserData->role_id == cn::STUDENT_ROLE_ID){
            $ExerciseCount = $this->GetExerciseCountByCurriculumYear($this->GetCurriculumYear(),$this->id);
            return $ExerciseCount;
        }
    }

    /**
     * USE : Get All Attempted Self Learning Exam Count
     */
    public function getSelfLearningCountAttribute(){
        $SelfLearningCount = null;
        $UserData = Self::find($this->id);
        if(!empty($this->id) && isset($UserData->role_id) && $UserData->role_id == cn::STUDENT_ROLE_ID){
            $SelfLearningCount = $this->GetSelfLearningCountByCurriculumYear($this->GetCurriculumYear(),$this->id);
            return $SelfLearningCount;
        }
    }

    /**
     * USE : Get All Attempted Ai-based Assessment Exam Count
     */
    public function getAiBasedAssessmentCountAttribute(){
        $AiBasedAssessment = null;
        $UserData = Self::find($this->id);
        if(!empty($this->id) && isset($UserData->role_id) && $UserData->role_id == cn::STUDENT_ROLE_ID){
            $AiBasedAssessment = $this->GetAiBasedAssessmentCountByCurriculumYear($this->GetCurriculumYear(),$this->id);
            return $AiBasedAssessment;
        }
    }
