...
class StudentController extends Controller
{
    ...
    public function getSelfLearningExerciseList(Request $request){
        try{
            ...
            $ExamsData['exercise_list'] =   Exam::with(['attempt_exams' => fn($query) => $query->where(cn::ATTEMPT_EXAMS_STUDENT_STUDENT_ID, $userId)])
                                            ->whereRaw("find_in_set($userId,student_ids)")
                                            ->where([
                                                cn::EXAM_TYPE_COLS => 1,
                                                cn::EXAM_TABLE_IS_GROUP_TEST_COL => 0,
                                                cn::EXAM_TABLE_CREATED_BY_COL => $studentId,
                                                cn::EXAM_TABLE_SELF_LEARNING_TEST_TYPE_COL => 1,
                                                cn::EXAM_TABLE_STATUS_COLS => 'publish',
                                                cn::EXAM_CURRICULUM_YEAR_ID_COL=>$this->GetCurriculumYear()
                                            ])
            ...
            if(Auth::user()->role_id != 3){
                return view('backend/exams/student_self_learning_exercise_exam_list',compact('ExamsData','difficultyLevels','active_tab','schoolId','userId','roleId','studentId','studentData'));
            }else{
    ...
    public function getTestingZoneList(Request $request){
        try{
            ...
            //                                 ->orderBy(cn::EXAM_TABLE_CREATED_AT,'DESC')
            //                                 ->get();
            $ExamsData['test_list'] = Exam::with(['attempt_exams' => fn($query) => $query->where(cn::ATTEMPT_EXAMS_STUDENT_STUDENT_ID, $userId)])
                                            ->whereRaw("find_in_set($userId,student_ids)")
                                            ->where([
                                                cn::EXAM_TYPE_COLS => 1,
                                                cn::EXAM_TABLE_IS_GROUP_TEST_COL => 0,
                                                cn::EXAM_TABLE_CREATED_BY_COL => $userId,
                                                cn::EXAM_TABLE_SELF_LEARNING_TEST_TYPE_COL => 2,
                                                cn::EXAM_TABLE_STATUS_COLS => 'publish',
                                                cn::EXAM_CURRICULUM_YEAR_ID_COL => $this->GetCurriculumYear()
                                            ])
                                            ...
                                            ->get();
            if(Auth::user()->role_id != 3){
                return view('backend/exams/student_self_learning_test_exam_list',compact('ExamsData','difficultyLevels','schoolId','userId','roleId','studentId','studentData'));
            }else{
